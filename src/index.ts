...
import os from "os";
import { createEmailMessage, adaptMessagePart } from "./utl.js";
...
function adaptMessagePart(part: Schema$MessagePart): GmailMessagePart {
  return {
...
const ForwardEmailSchema = z.object({
  ...
  to: z
    .array(z.string())
    .describe("List of recipient email addresses to forward to"),
  cc: z.array(z.string()).optional().describe("List of CC recipients"),
  bcc: z.array(z.string()).optional().describe("List of BCC recipients"),
  body: z
    .string()
    .optional()
    .describe("Optional additional message to include in the forwarded email"),
});
...
async function main() {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: "forward_email",
        description: "Forwards an existing email to specified recipients",
    ...
    try {
      switch (name) {
        ...
        case "forward_email": {
          ...
          const adaptedPayload = payload ? adaptMessagePart(payload) : {};

          // 提取邮件正文（纯文本或 HTML）
          const { text, html } = extractEmailContent(adaptedPayload);
          ...
          // 构建转发邮件内容
          const forwardBody = validatedArgs.body
            ? `${validatedArgs.body}\n\n--- Forwarded Message ---\nFrom: ${from}\nSubject: ${subject}\n\n${originalContent}`
            : `--- Forwarded Message ---\nFrom: ${from}\nSubject: ${subject}\n\n${originalContent}`;
          ...
          return {
            content: [
              {
                type: "text",
                text: `Email forwarded successfully with ID: ${sendResponse.data.id}`,
              },
            ],
          };
        }
        ...
        case "list_email_labels": {
          ...
          const systemLabels = formattedLabels.filter(
            (label) => label.type === "system",
          );
          const userLabels = formattedLabels.filter(
